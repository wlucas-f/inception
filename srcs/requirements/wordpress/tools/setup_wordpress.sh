...
# -------------------------------------------------------------------
# Read WordPress second user password from Docker secret
# -------------------------------------------------------------------
WP_USER_PASSWORD_FILE="/run/secrets/wp_user_password"
if [ ! -f "$WP_USER_PASSWORD_FILE" ]; then
    echo "ERROR: WordPress user password secret does not exist."
    exit 1
fi
WP_USER_PASSWORD="$(cat "$WP_USER_PASSWORD_FILE")"
if [ -z "$WP_USER_PASSWORD" ]; then
    echo "ERROR: WordPress user password is empty."
    exit 1
fi
...
# -------------------------------------------------------------------
# Create second (non-admin) user if it doesn't exist yet
# -------------------------------------------------------------------
if ! wp user get "${WP_USER:-editor}" --path="$WP_PATH" --allow-root >/dev/null 2>&1; then
    ...
    wp user create \
        ...
        --path="$WP_PATH" \
        --allow-root
    echo "Second user '${WP_USER:-editor}' created."
else
